...
const SettingsModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    const { technicians, addTechnician, deleteTechnician, updateTechnician, isSyncing } = useAppContext();
    ...
    const handleSave = async () => {
        ...
        const success = await addTechnician(newTech);
    ...
    const handleDeleteClick = async (techId: string, name: string) => {
        if (window.confirm(`⚠️ CONFIRM DELETION\n\nAre you sure you want to permanently remove technician "${name}"?\n\nThis will delete them from the server and all devices.`)) {
            setIsProcessing(true);
    ...
    const saveEdit = (tech: Technician) => {
        ...
        updateTechnician({ ...tech, name: editName, password: editPass });
        setEditingId(null);
    };
                ...
                <div className="p-6 overflow-y-auto space-y-6 bg-white relative">
                    {(isProcessing || isSyncing) && (
                        <div className="absolute inset-0 bg-white/60 z-10 flex items-center justify-center">
                            <div className="bg-white px-6 py-4 rounded-2xl shadow-xl border flex items-center space-x-3">
                                <div className="animate-spin h-5 w-5 border-2 border-glen-blue border-t-transparent rounded-full"></div>
                                <span className="text-sm font-bold text-glen-blue">Syncing with Server...</span>
                            </div>
                        </div>
                    )}

                    <div className="space-y-3">
                        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest px-1">Active Staff</h4>
                                    ...
                                    {editingId === tech.id ? (
                                        <div className="flex items-center space-x-3">
                                            <div className="flex-grow space-y-2">
                                                ...
                                                <input value={editPass} onChange={e => setEditPass(e.target.value)} className="w-full p-2 border rounded text-sm font-mono" />
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex space-x-2">
                                                <button onClick={() => startEditing(tech)} className="p-2 text-blue-400 hover:text-blue-600 rounded-lg hover:bg-blue-50">
                                                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                                </button>
                                                <button onClick={() => handleDeleteClick(tech.id, tech.name)} className="p-2 text-red-400 hover:text-red-600 rounded-lg hover:bg-red-50">
                        ...
                        <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100 space-y-4 shadow-inner">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input placeholder="Name" value={newTech.name} onChange={e => setNewTech({...newTech, name: e.target.value})} className="mt-1 w-full p-3 border border-gray-200 rounded-xl text-sm outline-none" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">PIN</label>
                                    <input type="tel" placeholder="3-4 Digits" value={newTech.password} onChange={e => setNewTech({...newTech, password: e.target.value.replace(/\D/g, '')})} maxLength={4} className="mt-1 w-full p-3 border border-gray-200 rounded-xl text-sm font-mono outline-none" />
                                </div>
                            </div>
                            <button onClick={handleSave} disabled={isProcessing} className="w-full bg-glen-blue text-white py-3 rounded-xl text-sm font-bold shadow-lg hover:bg-blue-600 transition-all flex items-center justify-center space-x-2">
